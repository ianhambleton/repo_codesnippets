...
log using logfiles\d013_barcharts_007, replace

**  GENERAL DO-FILE COMMENTS
**  program:      d013_barcharts_007.do
**  project:      
**  author:       HAMBLETON \ 12-NOV-2015
...
#delimit ;
	graph twoway 
		/// US-States (MEN)
		(bar inc2 loc4 if race==1 ,  							lc(blue*0.75) fc(blue*0.75))
		(bar inc2 loc4 if race==2,  							lc(blue*0.75) fc(blue*0.75))
		///(bar inc2 loc4 if race==1 & cid>=1000 & car==1,  		lc(red*0.75) fc(red*0.75))
		///bar inc2 loc4 if race==1 & cid>=1000 & non_car==1,  	lc(purple*0.75) fc(purple*0.75))
...
** GRAPHIC 2	TWO COLOURS, WHITE AMERICA, BLACK AMERICA
**				CARIBBEAN AVERAGES in PURPLE (nES) and RED (ES)
#delimit ;
	graph twoway 
		/// US-States (MEN)
		(bar inc2 loc4 if race==1 ,  							lc(blue*0.75) fc(blue*0.75))
		(bar inc2 loc4 if race==2,  							lc(orange*0.75) fc(orange*0.75))
		(bar inc2 loc4 if race==1 & cid>=1000 & car==1,  		lc(red*0.75) fc(red*0.75))
		(bar inc2 loc4 if race==1 & cid>=1000 & non_car==1,  	lc(purple*0.75) fc(purple*0.75))
