...
log using logfiles\a054_car_003, replace

**  GENERAL DO-FILE COMMENTS
**  program:      a054_car_003.do
	...
	** Collapse out country indicator. Leaves us with Regional data
	...
	forval sx = 1(1)3 {
			...
			distrate deaths pop using "data\input\us2000`age2'" if sex==`sx' & decade==`yr', 	///
				...
				saving(data\result\car_003\mr_car_`var'_`sx'_`yr'_`drop'`age2'`quality', replace)
				}
			}
		}
...
foreach var in `cod' {
	use "data\result\car_003\mr_car_`var'_1_1_`drop'`age2'`quality'.dta", clear		
	gen cod = `k'
	gen decade = 1
	gen sex = 1			
	save "data\result\car_003\mr_car_`var'_1_1_`drop'`age2'`quality'.dta", replace		
	local k=`k'+1
	}
local k=2
** Female. Decade 2 (2009-2011)
foreach var in `cod' {
	use "data\result\car_003\mr_car_`var'_1_2_`drop'`age2'`quality'.dta", clear		
	gen cod = `k'
	gen decade = 2
	gen sex = 1
	save "data\result\car_003\mr_car_`var'_1_2_`drop'`age2'`quality'.dta", replace		
	local k=`k'+1
	}
local k=2
** Male. Decade 1 (1999-2001)
foreach var in `cod' {
	use "data\result\car_003\mr_car_`var'_2_1_`drop'`age2'`quality'.dta", clear		
	gen cod = `k'
	gen decade = 1
	gen sex = 2
	save "data\result\car_003\mr_car_`var'_2_1_`drop'`age2'`quality'.dta", replace		
	local k=`k'+1
	}
local k=2
** Male. Decade 2 (2009-2011)
foreach var in `cod' {
	use "data\result\car_003\mr_car_`var'_2_2_`drop'`age2'`quality'.dta", clear		
	gen cod = `k'
	gen decade = 2
	gen sex = 2
	save "data\result\car_003\mr_car_`var'_2_2_`drop'`age2'`quality'.dta", replace		
	local k=`k'+1
	}
local k=2
** Female+Male. Decade 1 (1999-2001)
foreach var in `cod' {
	use "data\result\car_003\mr_car_`var'_3_1_`drop'`age2'`quality'.dta", clear		
	gen cod = `k'
	gen decade = 1
	gen sex = 3
	save "data\result\car_003\mr_car_`var'_3_1_`drop'`age2'`quality'.dta", replace		
	local k=`k'+1
	}
local k=2
** Female+Male. Decade 2 (2009-2011)
foreach var in `cod' {
	use "data\result\car_003\mr_car_`var'_3_2_`drop'`age2'`quality'.dta", clear		
	gen cod = `k'
	gen decade = 2
	gen sex = 3
	save "data\result\car_003\mr_car_`var'_3_2_`drop'`age2'`quality'.dta", replace		
	local k=`k'+1
	}
	
** Create a single dataset
use "data\result\car_003\mr_car_cod2_1_1_`drop'`age2'`quality'.dta"	, clear	

foreach var in `cod' {
	append using "data\result\car_003\mr_car_`var'_1_1_`drop'`age2'`quality'.dta"		
	}
foreach var in `cod' {
	append using "data\result\car_003\mr_car_`var'_1_2_`drop'`age2'`quality'.dta"		
	}
foreach var in `cod' {
	append using "data\result\car_003\mr_car_`var'_2_1_`drop'`age2'`quality'.dta"		
	}
foreach var in `cod' {
	append using "data\result\car_003\mr_car_`var'_2_2_`drop'`age2'`quality'.dta"		
	}
foreach var in `cod' {
	append using "data\result\car_003\mr_car_`var'_3_1_`drop'`age2'`quality'.dta"		
	}
foreach var in `cod' {
	append using "data\result\car_003\mr_car_`var'_3_2_`drop'`age2'`quality'.dta"		
	}
...
save "data\result\car_003\mr_car`age2'`quality'.dta", replace
		...
		forval sx = 1(1)3 {
			forval yr = 1(1)2 {
				...
				distrate deaths pop using "data\input\us2000`age2'" if cid==`cid' & sex==`sx' & decade==`yr', 	///
					stand(age11) popstand(prop) mult(100000) format(%8.1f)	///
					saving(data\result\car_003\mr_`cid'_`var'_`sx'_`yr'_`drop'`age2'`quality', replace)
					}
				}
			}
		}
...
foreach cid in `country' {
	local k=2
	foreach var in `cod' {
		use "data\result\car_003\mr_`cid'_`var'_1_1_`drop'`age2'`quality'.dta", clear		
		cap drop cod year sex
		gen cid = `cid'
		gen cod = `k'
		gen decade = 1
		gen sex = 1			
		save "data\result\car_003\mr_`cid'_`var'_1_1_`drop'`age2'`quality'.dta", replace		
		local k=`k'+1
		}
	}
** Female. Decade 2 (2009-2011)
foreach cid in `country' {
	local k=2
	foreach var in `cod' {
	use "data\result\car_003\mr_`cid'_`var'_1_2_`drop'`age2'`quality'.dta", clear		
		cap drop cod year sex
		gen cid = `cid'
		gen cod = `k'
		gen decade = 2
		gen sex = 1
		save "data\result\car_003\mr_`cid'_`var'_1_2_`drop'`age2'`quality'.dta", replace		
		local k=`k'+1
		}
	}
** Male. Decade 1 (1999-2001)
foreach cid in `country' {
	local k=2
	foreach var in `cod' {
		use "data\result\car_003\mr_`cid'_`var'_2_1_`drop'`age2'`quality'.dta", clear		
		cap drop cod year sex
		gen cid = `cid'
		gen cod = `k'
		gen decade = 1
		gen sex = 2
		save "data\result\car_003\mr_`cid'_`var'_2_1_`drop'`age2'`quality'.dta", replace		
		local k=`k'+1
		}
	}
** Male. Decade 2 (2009-2011)
foreach cid in `country' {
	local k=2
	foreach var in `cod' {
		use "data\result\car_003\mr_`cid'_`var'_2_2_`drop'`age2'`quality'.dta", clear		
		cap drop cod year sex
		gen cid = `cid'
		gen cod = `k'
		gen decade = 2
		gen sex = 2
		save "data\result\car_003\mr_`cid'_`var'_2_2_`drop'`age2'`quality'.dta", replace		
		local k=`k'+1
		}
	}
** Female+Male. Decade 1 (1999-2001)
foreach cid in `country' {
	local k=2
	foreach var in `cod' {
		use "data\result\car_003\mr_`cid'_`var'_3_1_`drop'`age2'`quality'.dta", clear		
		cap drop cod year sex
		gen cid = `cid'
		gen cod = `k'
		gen decade = 1
		gen sex = 3
		save "data\result\car_003\mr_`cid'_`var'_3_1_`drop'`age2'`quality'.dta", replace		
		local k=`k'+1
		}
	}
** Female+Male. Decade 2 (2009-2011)
foreach cid in `country' {
	local k=2
	foreach var in `cod' {
		use "data\result\car_003\mr_`cid'_`var'_3_2_`drop'`age2'`quality'.dta", clear		
		cap drop cod year sex
		gen cid = `cid'
		gen cod = `k'
		gen decade = 2
		gen sex = 3
		save "data\result\car_003\mr_`cid'_`var'_3_2_`drop'`age2'`quality'.dta", replace		
		local k=`k'+1
		}
	}

** Create a single dataset (NB cid in this first example = Antigua)
use "data\result\car_003\mr_2010_cod2_1_1_`drop'`age2'`quality'.dta"	, clear	
** Use when restricting to Non English speaking caribbean
** use "data\result\car_003\mr_2025_cod2_1_1_`drop'`age2'`quality'.dta"	, clear	
** Use when restricting to Non English speaking caribbean (<25% undercount)
** use "data\result\car_003\mr_2150_cod2_1_1_`drop'`age2'`quality'.dta"	, clear	
foreach cid in `country' {
	foreach var in `cod' {
		append using "data\result\car_003\mr_`cid'_`var'_1_1_`drop'`age2'`quality'.dta"		
		}
	}
foreach cid in `country' {
	foreach var in `cod' {
		append using "data\result\car_003\mr_`cid'_`var'_1_2_`drop'`age2'`quality'.dta"		
		}
	}
foreach cid in `country' {
	foreach var in `cod' {
		append using "data\result\car_003\mr_`cid'_`var'_2_1_`drop'`age2'`quality'.dta"		
		}
	}
foreach cid in `country' {
	foreach var in `cod' {
		append using "data\result\car_003\mr_`cid'_`var'_2_2_`drop'`age2'`quality'.dta"		
		}
	}
foreach cid in `country' {
	foreach var in `cod' {
		append using "data\result\car_003\mr_`cid'_`var'_3_1_`drop'`age2'`quality'.dta"		
		}
	}
foreach cid in `country' {
	foreach var in `cod' {
		append using "data\result\car_003\mr_`cid'_`var'_3_2_`drop'`age2'`quality'.dta"		
		}
	}
** Drop first row, which is repeated in row 2
drop if _n==1
save "data\result\car_003\mr_`drop'`age2'`quality'", replace
...
use "data\result\car_003\mr_car`age2'`quality'.dta", clear
gen cid=1000
append using "data\result\car_003\mr_`drop'`age2'`quality'"
** label country
#delimit ;
label define cid	 	
						1000 "Caribbean"
...
#delimit cr
save "data\result\car_003\mr_`name'_`drop'`age2'`quality'", replace
